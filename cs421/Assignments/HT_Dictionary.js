...
class Chain {
  ...
  findElem(key) {
      ...
      if (this.isLast(prev) || prev._next.key() != key) {
          ...
          let e = prev._next.element();
          // this.removeNext(prev);// Error Removed 11/11/20
          return e;
      }
  }
  toString() {
...
class HT_Dictionary {// eslint-disable-line no-unused-vars, camelcase
  // implemented as a linear hash table
  constructor () {
      this._table = [];
      this._size = 0;
      this._numChains = 3;
...
function genData(n, k) {// eslint-disable-line no-unused-vars
